...
use crate::nostr::{chaincode_string, pubkey_string, secret_key_string_short, Nip41};
...
fn usage() {
    ...
    println!("{} verify <invalid> <new> <new_hid>  \t Verify key invalidation; 2 pubkeys must be supplied (npub or hex) and a chaincode (hex)", progname);
    println!(
...
fn do_inv(commit: bool) {
    if let Some(mut state) = load_state() {
        ...
        println!(
            "Invalidated:       \t {}",
            pubkey_string(&inv_info.invalid_vis)
        );
        println!("        new:       \t {}", pubkey_string(&inv_info.new_vis));
        println!(
            "     hidden:       \t {}",
            chaincode_string(&inv_info.new_hid)
        );
        if commit {
            print_current(&state);
        }
...
fn do_verify_int(invalid_str: &str, new_str: &str, new_hid_str: &str) -> Result<bool, Error> {
    let invalid = Keys::from_pk_str(invalid_str)?.public_key();
    let new = Keys::from_pk_str(new_str)?.public_key();
    let new_hid = ChainCode::try_from(&hex::decode(new_hid_str)?)?;

    println!("Invalid vis     \t {}", pubkey_string(&invalid));
    println!("New vis         \t {}", pubkey_string(&new));
    println!("New hid         \t {}", chaincode_string(&new_hid));
    Ok(KeyManager::default().verify(&invalid, &new, &new_hid))
}

fn do_verify(invalid: &str, new: &str, new_hid: &str) {
    match do_verify_int(invalid, new, new_hid) {
