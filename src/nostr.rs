...
pub fn chaincode_string(cc: &ChainCode) -> String {
    format!("{}", hex::encode(cc.to_bytes()))
}
...
impl Nip41 {
        ...
        Ok(EventBuilder::new(
            ...
            &[
                ...
                Tag::Generic(
                    ...
                    vec![hex::encode(invalid_hid.to_bytes())],
                ),
            ],
        ))
    }
    ...
    pub fn build_invalidate_event_unsigned(
        ...
        let eb = Self::invalidate_event_builder(invalid_vis, new_hid)?;
        let event = eb.to_unsigned_event(new_vis);
        Ok(event)
    }
    ...
    pub fn verify(event: &Event) -> Result<bool, String> {
                ...
                Tag::Generic(tagkind, vals) => {
                    if let TagKind::Custom(s) = tagkind {
                        if s == "hidden-key" && vals.len() > 0 {
                            let val_bytes = hex::decode(&vals[0])
                                .map_err(|_| "Invalid hex string".to_string())?;
                            let chaincode = ChainCode::try_from(&val_bytes)
                                .map_err(|_| "Invalid hex string".to_string())?;
                            tag_hidden_key = Some(chaincode);
                        }
                    }
                }
                _ => {}
        ...
        let invalid = tag_p.unwrap().clone();
        let new_hid = tag_hidden_key.unwrap();
        ...
        println!(
            ...
            chaincode_string(&new_hid)
        );

        if !KeyManager::default().verify(&invalid, &new_pk, &new_hid) {
            Ok(false)
        } else {
            println!(
    ...
    pub fn build_invalidate_event_from_state(state: &KeyState) -> Result<Event, Error> {
        ...
        let eb = Self::invalidate_event_builder(inv_info.invalid_vis, inv_info.new_hid)?;
...
mod test {
    ...
    fn test_build_invalidate_event() {
        ...
        assert_eq!(event_json, "{\"content\":\"key invalidation\",\"created_at\":1680359581,\"id\":\"8f53bcce2f98d5900a9ab4c5c370409f29e5e6c334e4bd12053c742524927c50\",\"kind\":13,\"pubkey\":\"21fc3830dfe33b679456257c353e0b388a2ce323dfb7d19bc671bd31d0491b00\",\"tags\":[[\"p\",\"6a123fef0f52b6bd6a188b93f0322f3c828a8815cf83a106816bc343d2e4e5dd\"],[\"hidden-key\",\"3e5ea95edb1a94a3c75d822fffd8e3ffefac57a70e967bf3f01efd4222f288c0\"]]}");
    }
}
